...
### 14. В чем отличие Funcstring от Expression<Funcstring?
- `Func` — содержит **делегат (исполняемый код)**.
    
- `ExpressionFunc` — содержит **дерево выражений (описание кода)**.  
    Пример: `x => x.Age > 18`
    
- `Func` выполняется,
    
- `ExpressionFunc` — может быть проанализирован (например, ORM его переводит в SQL).
